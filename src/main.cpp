...
// =============================================================
// USB HID Joystick (TinyUSB custom descriptor, via ESP32-S3 USB-OTG)
// 6 axes @ 16-bit resolution (-32767..32767) + 16 buttons + hat
// =============================================================
static HIDJoystick16 Joystick;
...
// =============================================================
// Map RC pulse (µs) → signed HID axis (-32767 … +32767, int16_t)
// =============================================================
...
void loop() {
    ...
    if (now - lastHidMs >= 10) {
        ...
        // send(x, y, z, rx, ry, rz, buttons, hat)
        Joystick.send(
            rcToAxis(ch[0]),  // CH1 — Aileron  (X)
            rcToAxis(ch[1]),  // CH2 — Elevator (Y)
            rcToAxis(ch[2]),  // CH3 — Throttle  (Z)
            rcToAxis(ch[3]),  // CH4 — Rudder    (Rx)
            rcToAxis(ch[4]),  // CH5 — AUX1      (Ry)
            rcToAxis(ch[5]),  // CH6 — AUX2      (Rz)
            0,                // buttons — none pressed
            JOYSTICK16_HAT_CENTER
        );
    }
}
